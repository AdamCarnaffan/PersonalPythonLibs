...
class Matrix:
    ...
    def __add__(self, other):
        if (self.rows == other.rows) and (self.cols == other.cols):
            ...
            for r in range(0, self.rows, 1):
                for c in range(0, self.cols, 1):
                    finalM.setValue(r, c, self.M[r][c] + other.M[r][c], True)
            return finalM
        else:
    ...
    def swapRows(self, r1, r2):
        ...
        self.setRow(r1, row2)
        # swap row labels
        rowL1 = self.rowLabels[r1]
        self.rowLabels[r1] = self.rowLabels[r2]
    ...
    def cutMatrix(self, step):
        ...
        dup = self.duplicate()
        if dup.rows == 1 or dup.cols == 1:
            return False
        for i in range(0, step, 1):
            dup = Matrix(dup.M[1:dup.rows])
            for r in dup.M:
                r = r[1:dup.cols]
            dup.cols = dup.cols - 1
        return dup
    ...
    def geFwd(self):
        ...
        for r in range(0, dup.rows, 1):
            ...
            if dup.M[r][r] == 0:
                ind = 0
                for i in range(r, dup.rows, 1):
                    if dup.M[i][r] != 0:
                        dup.swapRows(r, i)
                        dup.swaps = dup.swaps + 1
                        break
            first = dup.M[r]
            for i in range(r+1, dup.rows, 1):
                if (dup.M[i][r] == 0):
                    continue
                # Scale row
                sc = Matrix([first])
                sc.setDataType('float')
                sc.scale(-(dup.M[i][r]/sc.M[0][r])) # For subtraction
                rowM = Matrix([dup.M[i]])
                rowM.setDataType('float')
                finalRow = rowM + sc
                #finalRow.display()
                # if finalRow.checkZeroRow(0):
                #     print("---")
                #     sc.display()
                #     rowM.display()
                #     print("---")
                dup.setRow(i, finalRow.M[0])
                #print(dup.M[len(dup.M)-1])
            # Move zero rows to bottom
            ind = 0
            for i in range(0, self.rows, 1):
                isZero = dup.checkZeroRow(i)
                if isZero:
                    dup.bumpRow(ind)
                ind = ind + 1
            #print(dup.M[len(dup.M)-1])
    ...
    def bumpRow(self, row):
        # Adjust row
        self.M = self.M[0:row] + self.M[row+1:len(self.M)] + [self.M[row]]
        # Adjust row label
